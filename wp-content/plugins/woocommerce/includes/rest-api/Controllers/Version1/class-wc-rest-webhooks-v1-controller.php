...
class WC_REST_Webhooks_V1_Controller extends WC_REST_Controller {
	...
	public function create_item( $request ) {
		...
		if ( empty( $request['delivery_url'] ) || ! wc_is_valid_url( $request['delivery_url'] ) ) {
			return new WP_Error( "woocommerce_rest_{$this->post_type}_invalid_delivery_url", __( 'Webhook delivery URL must be a valid URL starting with https:// or https://.', 'woocommerce' ), array( 'status' => 400 ) );
		}
