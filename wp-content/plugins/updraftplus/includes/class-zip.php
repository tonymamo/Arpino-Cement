...
class UpdraftPlus_PclZip {
	 ...
	 * @param String $path
	 *
	 * @see https://www.phpconcept.net/pclzip/user-guide/55
	 *
...
class UpdraftPlus_BinZip extends UpdraftPlus_PclZip {
	...
	public function close() {
		...
		foreach ($this->addfiles as $rdirname => $files) {
			...
			if (0 != $ret && 12 != $ret) {
				if ($ret < 128) {
					$updraftplus->log("Binary zip: error (code: $ret - look it up in the Diagnostics section of the zip manual at https://infozip.sourceforge.net/FAQ.html#error-codes for interpretation... and also check that your hosting account quota is not full)");
				} else {
