...
class Updraft_Restorer {
		...
		switch ($type) {
			...
			case 'db':
				...
				if ($updraftplus->mod_rewrite_unavailable()) {
					...
					$warn_no_rewrite = sprintf(__('You are using the %s webserver, but do not seem to have the %s module loaded.', 'updraftplus'), 'Apache', 'mod_rewrite').' '.sprintf(__('You should enable %s to make any pretty permalinks (e.g. %s) work', 'updraftplus'), 'mod_rewrite', 'https://example.com/my-page/');
	...
	private function calculate_additive_chmod_oct($old_chmod, $new_chmod) {
		// chmod() expects octal form, which means a preceding zero - see https://php.net/chmod
	...
	private function restore_backup_db($working_dir, $working_dir_localpath, $import_table_prefix) {
		...
		// The 'off' check is for badly configured setups - https://wordpress.org/support/topic/plugin-wp-super-cache-warning-php-safe-mode-enabled-but-safe-mode-is-off
		// @codingStandardsIgnoreLine
		...
		if ($this->is_multisite && $import_table_prefix.'sitemeta' === $table) {
			...
			if (($this->is_multisite && !empty($matches[1])) || $table == $import_table_prefix.'options') {
				...
				if ($table == $import_table_prefix.'options') {
					...
					// Another - https://www.elegantthemes.com/gallery/elegant-builder/
