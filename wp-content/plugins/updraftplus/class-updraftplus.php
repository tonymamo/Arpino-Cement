...
class UpdraftPlus {
	...
	public function get_updraftplus_rssfeed() {
		...
		return fetch_feed('https://feeds.feedburner.com/updraftplus/');
	}
				...
				if ('' == $old_siteurl && preg_match('/^\# Backup of: (http(.*))$/', $buffer, $matches)) {
					...
					if (untrailingslashit(site_url()) != $old_siteurl) {
						...
						if ($this->mod_rewrite_unavailable(false)) {
							$warn[] = sprintf(__('You are using the %s webserver, but do not seem to have the %s module loaded.', 'updraftplus'), 'Apache', 'mod_rewrite').' '.sprintf(__('You should enable %s to make any pretty permalinks (e.g. %s) work', 'updraftplus'), 'mod_rewrite', 'https://example.com/my-page/');
						}
	...
	public function get_outgoing_ip_address($use_ipv6_service = false) {
		...
		if ($use_ipv6_service) array_unshift($urls, 'https://ip6.me/api');
